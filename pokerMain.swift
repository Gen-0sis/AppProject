...
class Manager {
    ...
    func convertToChips(amount amountToConvert: Int) -> [Chip: Int] {
        var bet = amountToConvert
...
struct pokerHands {
    ...
    var isAceHigh: Bool {
        let aceHigh = Set(["Ace", "2", "3", "4", "5"])
        return Set(cards.map { $0.face }) == aceHigh
    }

}

func runBettingRound(players: inout[Player], deck mainDeck: Deck, pile discardPile: Deck,ante anteAmount: Int = 0, manager: Manager) -> Void {
...
func checkPlayerHand(hand: [Card]) -> PokerHand {
    ...
    if handEval.isFlush && handEval.isStraight && handEval.isRoyal {
        //royal flush
        return .royalFlush
    } else if handEval.isFlush && (handEval.isStraight || handEval.isAceHigh) {
...
//Takes ante bets from each player (REPLACE)
...
func checkStatus(players: [Player], manager: Manager) -> Bool {
    ...
    let activePlayers = players.filter { !$0.hand.isEmpty }.count
...
//contains winner evaluation logic. defers ties to the manager classes' compareHighCards function
func determineWinner(players: [Player], remainingPlayers activePlayers: [Player], manager: Manager) {
    // Assign hand rank to each player
    for player in activePlayers {
        player.handRank = manager.HAND_RANKS[player.handType] ?? 10 // Default to highCard
    }

    // Find the best hand rank (lowest number is better)
    let bestRank = activePlayers.map { $0.handRank }.min() ?? 10

    // Filter to players with best hand rank
    let contenders = activePlayers.filter { $0.handRank == bestRank }
    ...
    // If tie on hand rank, use high card (tieBreakerCard)
    var currentBestPlayer = contenders[0]
    for contender in contenders.dropFirst() {
        currentBestPlayer = manager.compareHighCards(playerOne: currentBestPlayer, playerTwo: contender)
    }
...
//CLI user interface, will replace
func main() {
